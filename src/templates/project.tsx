...
const Book: React.FunctionComponent<PageProps> = ({ data: { book, images } }) => {
	...
	return (
		<Layout color={book.color}>
			...
			<PBox py={10} px={[ 6, 6, 8, 10 ]}>
				<Category style={categoryAnimation}>{book.category}</Category>
				<animated.h1 style={titleAnimation}>{book.title_detail}</animated.h1>
				<Description style={descAnimation}>
					<div dangerouslySetInnerHTML={{ __html: book.desc }} />
				</Description>
			</PBox>
			<Content bg={book.color} py={10}>
...
export const query = graphql`
	query BookTemplate($slug: String!, $images: String!) {
		book: booksYaml(slug: { eq: $slug }) {
			title_detail
			color
			category
			desc
